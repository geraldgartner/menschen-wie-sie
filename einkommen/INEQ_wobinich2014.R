...
wogewsozalter<-filter(wogew, BLD =="Alle", GESCH=="Alle", ALTER == "Alle", BESCH=="Vollzeit", EDU=="Alle") #FACET FÜR SOZSTELL ANSEHEN
...
wogewalle <- filter(wogew, BLD =="Alle", ALTER == "Alle", SOZST=="Alle", BESCH=="Alle", EDU=="Alle")
wogewbildvolz <- filter(alle, BESCH=="Vollzeit") #

#Alle Verteilungen nach Bildung
g <- ggplot(wogewbildvolz, aes(x= PERZ, y = Freq)) +
  geom_bar(stat="identity") +
  facet_wrap(~ EDU, ncol=3)+
  theme#(strip.background = element_blank(),
#strip.text.x = element_blank())
print(g)
ggsave(g, file="einkommensverteilungbildung2014.pdf")
...
#strip.text.x = element_blank())
print(g)
ggsave(g, file="einkommensverteilungnachalter2014.pdf")
